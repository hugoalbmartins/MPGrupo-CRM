/*
# Update save_commission_configs RPC to include multiply_without_vat field

1. Modified Functions
   - `save_commission_configs` - added multiply_without_vat boolean field
     to the INSERT statement within the loop

2. Important Notes
   - multiply_without_vat defaults to false when not provided
   - When true and commission_mode is 'monthly_multiplier', the monthly value
     is divided by 1.23 before multiplying
*/
...
BEGIN
  ...
  LOOP
    INSERT INTO commission_configurations (
      ...
      v_config->>'technology',
      v_config->>'from_email',
      v_config->>'from_smtp_pass',
      COALESCE((v_config->>'multiply_without_vat')::boolean, false),
      auth.uid(),
      auth.uid()
    );
  END LOOP;
