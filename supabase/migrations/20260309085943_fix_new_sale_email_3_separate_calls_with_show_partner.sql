/*
  # Rewrite new sale email trigger - 3 separate calls with show_partner flag

  ## Changes

  Rewrites `create_new_sale_alert_with_email` to send 3 SEPARATE http_post calls
  instead of a single call with TO/BCC split:

  ### Call 1 - Admins / BO (show_partner = true)
  - Recipients: admin/bo users OR operator-specific users
  - show_partner = true (they can see the partner name)

  ### Call 2 - Partner / Vendedor (show_partner = true)
  - Recipients: partner user and the sale creator if they are partner/partner_commercial
  - show_partner = true (they ARE the partner, so it makes sense to show their name)

  ### Call 3 - Operator emails (show_partner = false)
  - Recipients: operator notification_emails (external addresses)
  - show_partner = false (operator emails should not expose partner info)

  ## Notes
  - Partners now correctly see partner_name in their email
  - Each call is independent so failures are isolated
  - D2D partners still excluded from call 2
*/
  ...
  IF jsonb_array_length(v_admin_recipients) > 0 THEN
    BEGIN
      PERFORM net.http_post(
        ...
        body := v_base_payload || jsonb_build_object(
