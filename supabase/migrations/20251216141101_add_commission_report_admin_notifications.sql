/*
  # Add Admin Notifications for Commission Reports

  1. New Function
    - `notify_admins_on_commission_report` - Trigger function to notify admins when report is sent

  2. Trigger
    - Calls edge function to send admin notifications when email_sent is updated to true

  3. Security
    - Uses secure service role to call edge function
*/

-- Function to notify admins when commission report is sent
...
DECLARE
  ...
  v_app_url TEXT;
BEGIN
  IF (TG_OP = 'UPDATE' AND NEW.email_sent = true AND (OLD.email_sent IS NULL OR OLD.email_sent = false)) THEN
...
DROP TRIGGER IF EXISTS trigger_notify_admins_on_commission_report ON commission_reports;
CREATE TRIGGER trigger_notify_admins_on_commission_report
  AFTER INSERT OR UPDATE ON commission_reports
  FOR EACH ROW
  EXECUTE FUNCTION notify_admins_on_commission_report();