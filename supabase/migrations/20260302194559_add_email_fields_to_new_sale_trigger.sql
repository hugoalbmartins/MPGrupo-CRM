/*
  # Add email_fields to new sale email trigger

  ## Summary
  Updates the create_new_sale_alert_with_email function and trigger_new_sale_alert
  to pass the operator's email_fields configuration to the send-new-sale-email edge function.

  ## Changes
  1. Drops and recreates create_new_sale_alert_with_email with new p_email_fields parameter
  2. Updates trigger_new_sale_alert to look up and pass email_fields from the operators table
*/
...
DROP FUNCTION IF EXISTS create_new_sale_alert_with_email(
  ...
  text, text, boolean, boolean, boolean, boolean, text, text, integer, jsonb, text
);

-- Step 3: Recreate function with email_fields parameter
...
-- Step 4: Recreate trigger function to look up and pass email_fields
...
BEGIN
  ...
  SELECT email_fields INTO v_email_fields
  FROM operators
