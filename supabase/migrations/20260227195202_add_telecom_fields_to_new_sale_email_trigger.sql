/*
  # Add telecom portability fields to new sale email trigger

  ## Summary
  Updates the DB trigger and the create_new_sale_alert_with_email function to pass
  the new telecom fields (service_type, activation_type, has_tv, has_net, has_lr,
  fix_ported, fix_number, fix_operator, mobile_count, mobile_numbers) to the
  send-new-sale-email edge function so they appear in the email body.

  ## Changes
  1. Drops and recreates create_new_sale_alert_with_email with new parameters
  2. Drops and recreates trigger_new_sale_alert to pass new fields from NEW row
*/

-- Step 1: Drop the trigger that calls the function
DROP TRIGGER IF EXISTS sales_new_sale_alert ON sales;

-- Step 2: Drop old function (has fixed parameter list)
...
-- Step 3: Recreate function with new telecom parameters
...
-- Step 4: Recreate trigger function to pass new fields
