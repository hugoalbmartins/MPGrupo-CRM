/*
  # Add fix_cvp to new sale email trigger

  ## Summary
  Updates the `create_new_sale_alert_with_email` function and
  `trigger_new_sale_alert` trigger to pass the `fix_cvp` field
  (CVP of the ported fixed line) to the email edge function.

  ## Changes

  ### Modified Functions
  - `create_new_sale_alert_with_email`: Added `p_fix_cvp` parameter,
    included in base payload sent to edge function.
  - `trigger_new_sale_alert`: Passes `NEW.fix_cvp` to the function.
*/
...
BEGIN
  ...
  -- CALL 3: Operator notification emails (external addresses) — always use global SMTP (no operator credentials)
