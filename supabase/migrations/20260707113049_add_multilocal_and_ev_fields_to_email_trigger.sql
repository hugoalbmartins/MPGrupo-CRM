/*
  # Add multilocal/multiponto and EV fields to email trigger

  ## Summary
  Updates the email trigger functions to pass sale_type, billing_address,
  energy_points_list (from sales_energy_points table), and electric vehicle
  fields to the email edge function. Without these, multilocal/multiponto
  sale emails lack CPE details, addresses, and entry types per point.

  ## Changes
  - `create_new_sale_alert_with_email`: adds parameters for sale_type,
    billing_address, energy_points_list, ev_outlet_count, ev_monthly_fee,
    ev_margin, ev_fidelization_months and includes them in the HTTP POST body
  - `trigger_new_sale_alert`: queries sales_energy_points for the sale,
    builds the JSON array, and passes all new fields to the function
*/
...
DROP FUNCTION IF EXISTS create_new_sale_alert_with_email(
  ...
  text, text, text, text, text, text, numeric, numeric, numeric, boolean, boolean, text, text, text, text, text, text, boolean, text
);
...
DECLARE
  ...
  v_base_payload_no_attachments jsonb;
...
BEGIN
  ...
  v_base_payload_no_attachments := v_base_payload || jsonb_build_object('attachments', '[]'::jsonb);
  ...
  -- CALL 1: Admins/BO ou operator-specific users (show_partner = true, SEM anexos)
  ...
  IF jsonb_array_length(v_admin_recipients) > 0 THEN
    BEGIN
      PERFORM net.http_post(
        ...
        body := v_base_payload_no_attachments || v_email_extras || jsonb_build_object(
  ...
  -- CALL 2: Parceiro / vendedor (show_partner = true, SEM anexos)
  ...
  -- CALL 3: Operator notification emails (externos) — COM anexos, usar SMTP da operadora
...
BEGIN
  ...
  -- Build energy_points_list for multiponto/multilocal sales
  IF NEW.sale_type IN ('multiponto', 'multilocal') THEN
    FOR v_point IN
