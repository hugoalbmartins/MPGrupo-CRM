/*
  # Fix operator notification emails to use operator SMTP credentials

  ## Summary
  The 3rd email call (to operator notification_emails addresses) was not including
  the operator SMTP credentials (v_email_extras), causing it to fall back to global
  SMTP (info@mpgrupo.pt) instead of the configured operator email (e.g. telecom@mpgrupo.pt).

  ## Changes
  - Rebuilt `create_new_sale_alert_with_email` to include `v_email_extras` in CALL 3
  - All 3 email calls now use the operator SMTP when configured, global SMTP otherwise
  - Behaviour: CALL 1 (admins/BO) = operator SMTP, CALL 2 (partner) = operator SMTP,
    CALL 3 (notification_emails) = operator SMTP. All send from same address.

  ## Notes
  - Only the trigger function signature and CALL 3 body are changed; all other logic is identical
*/
...
BEGIN
  ...
  -- CALL 3: Operator notification emails — use operator SMTP when configured (same as calls 1 and 2)
