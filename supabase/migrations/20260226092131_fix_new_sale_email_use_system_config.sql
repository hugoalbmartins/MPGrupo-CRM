/*
  # Fix new sale email trigger to use system_config table

  ## Problem
  The `create_new_sale_alert_with_email` function reads the Supabase URL and anon key
  using `current_setting('app.settings.supabase_url', true)` which returns NULL because
  those PostgreSQL session settings are never populated.

  The `system_config` table already contains the correct values:
  - supabase_url = https://iydhpyljcofpztrzjnfr.supabase.co
  - supabase_anon_key = (correct JWT)

  ## Fix
  Rewrite the function to read URL and anon key from `system_config` table,
