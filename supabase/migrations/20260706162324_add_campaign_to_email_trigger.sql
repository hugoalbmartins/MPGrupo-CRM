/*
  # Add campaign field to new sale email trigger

  ## Summary
  Updates the email trigger functions to pass the sale's campaign field
  to the email edge function so it appears in notification emails.

  ## Changes
  - `create_new_sale_alert_with_email`: adds p_campaign text parameter
    and includes it in the email payload
  - `trigger_new_sale_alert`: passes NEW.campaign to the function
*/
...
DROP FUNCTION IF EXISTS create_new_sale_alert_with_email(
  ...
  text, text, text, text, text, text, numeric, numeric, numeric, boolean, boolean, text, text, text, text, text, text, boolean
);
