/*
  # Corrigir Função get_partners_with_sales_for_month

  ## Resumo
  Corrige a função para usar os nomes corretos das colunas da tabela sales:
  - `payment_date` ao invés de `paid_date`
  - Remove referência a `activation_date` que não existe
  
  ## Alteração
  Atualiza a função para usar as colunas corretas da tabela sales
*/
...
BEGIN
  ...
  eligible_sales AS (
    ...
    WHERE s.paid_to_operator = true
      AND (
        -- Usar data de pagamento se disponível, senão usar data da venda
