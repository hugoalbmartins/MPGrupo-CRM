/*
  # Add operator_requires_additional_services to new sale email trigger

  ## Summary
  Updates the new sale email trigger function to pass the operator's
  requires_additional_services flag to the email edge function, so that
  additional_services is always shown in the email (even when "Nenhum")
  when the operator has this feature enabled.

  ## Changes
  - `create_new_sale_alert_with_email`: adds p_operator_requires_additional_services param
    and includes it in the email payload
  - `trigger_new_sale_alert`: reads requires_additional_services from operator and passes it
*/
...
DROP FUNCTION IF EXISTS create_new_sale_alert_with_email(
  ...
  text, text, text, text, text, text, numeric, numeric, numeric, boolean, boolean, text, text, text, text, text
);
...
BEGIN
  ...
  -- Payload base COM anexos (apenas para CALL 3 - operadora)
  ...
  -- Payload base SEM anexos (para CALL 1 e CALL 2 - admins/BO/parceiro)
